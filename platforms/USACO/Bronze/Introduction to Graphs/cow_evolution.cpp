...
- If the number of different characteristics between the two sub-populations equals the number of characteristics in each of them, I concluded that they have different ancestor characteristics.
- Furthermore, if there are no two cows with the same number of characteristics, it would imply that each sub-population belongs exclusively to one of the two children of the root in the evolutionary tree.
However this approach is insufficient to determine the proper evolutionary tree in all cases. */

/* It is crucial to be aware of the condition for constructing a proper tree, which states that an evolutionary tree is called "proper" if each newly evolved feature originates in exactly one edge of the tree,
meaning it evolved at a single point in history. This condition ensures that the evolutionary tree accurately represents the lineage and development of characteristics within the bovine population. 
